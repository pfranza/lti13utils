...
public class LTI13JWSParser {
	...
	public Optional<String> getLTI3RequestErrors(Jws<Claims> jws) {
		...
		String ltiVersion = jws.getBody().get(LTI13Strings.LTI_VERSION, String.class);
		if (ltiVersion == null) {
			return Optional.of("Missing version");
		} else if (ltiVersion != null && !LTI13Strings.LTI_VERSION_3.equals(ltiVersion)) {
			return Optional.of("Incorrect version: " + ltiVersion);
		}

		String ltiMessageType = jws.getBody().get(LTI13Strings.LTI_MESSAGE_TYPE, String.class);
		if (ltiMessageType == null) {
			return Optional.of("Missing message type");
		} else if (ltiMessageType != null && !LTI13Strings.LTI_MESSAGE_TYPE_RESOURCE_LINK.equals(ltiMessageType)) {
